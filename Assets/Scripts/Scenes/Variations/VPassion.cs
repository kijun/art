...
public class VPassion: MonoBehaviour {
...
//        StartCoroutine(RunShapes());
    ...
    IEnumerator RunCamera() {
        ...
        var restChoices = new float[]{2*4, 3*4, 4*4, 6*4, 8*4};
        var longRestChoices = new float[]{5*4, 6*4};
    ...
    IEnumerator RunCameraPosition() {
        var restChoices = new float[]{2.1f*4, 3.1f*4};
    ...
    IEnumerator RunCameraZoom() {
        ...
        var restChoices = new float[]{3.25f*4, 4.75f*4};
